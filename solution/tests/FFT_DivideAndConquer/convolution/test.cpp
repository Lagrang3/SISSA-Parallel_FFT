...
int main(){
	...
	auto FA = FFT_DivideAndConquer(A, cd(cos(2*PI/N),sin(2*PI/N)) );
	auto FB = FFT_DivideAndConquer(B, cd(cos(2*PI/N),sin(2*PI/N)) );
	
	vector<cd> FC(N,0);
	for(int i=0;i<N;++i)
		FC[i]=FA[i]*FB[i];
	
	auto C2 = FFT_DivideAndConquer(FC,cd(cos(2*PI/N),-sin(2*PI/N)));
	double inv_n=1.0/N;
	for(auto &x: C2)
		x *= inv_n;
	// done
